export const POST_OAUTH_TOKEN_REQUEST = 'POST_OAUTH_TOKEN_REQUEST';
export const POST_OAUTH_TOKEN_SUCCESS = 'POST_OAUTH_TOKEN_SUCCESS';
export const POST_OAUTH_TOKEN_FAILURE = 'POST_OAUTH_TOKEN_FAILURE';
export const DELETE_OAUTH_TOKEN = 'DELETE_OAUTH_TOKEN';

export const POST_USER_SUCCESS = 'POST_USER_SUCCESS';
export const POST_USER_FAILURE = 'POST_USER_FAILURE';


export const GET_USER_FAILURE = 'GET_USER_FAILURE';
export const GET_USER_SUCCESS = 'GET_USER_SUCCESS';
export const GET_CURRENT_USER_FAILURE = 'GET_CURRENT_USER_FAILURE';
export const GET_CURRENT_USER_SUCCESS = 'GET_CURRENT_USER_SUCCESS'

export const FETCH_QUESTIONS_SUCCESS = 'FETCH_QUESTIONS_SUCCESS';
export const FETCH_QUESTIONS_FAILURE = 'FETCH_QUESTIONS_FAILURE';

export const POST_PLAN_SUCCESS = 'POST_PLAN_SUCCESS';
export const POST_PLAN_FAILURE = 'POST_PLAN_FAILURE';

export const NEXT_STEP = 'NEXT_STEP'
export const PREV_STEP = 'PREV_STEP'


export const PRESERV_ANSWER = 'PRESERV_ANSWER';

export const FETCH_RECOMMENDED_LOCATIONS_SUCCESS = 'FETCH_RECOMMENDED_LOCATIONS_SUCCESS'
export const FETCH_RECOMMENDED_LOCATIONS_FAILURE = 'FETCH_RECOMMENDED_LOCATIONS_FAILURE'

export const FETCH_LOCATION_FAILURE = 'FETCH_LOCATION_FAILURE'
export const FETCH_LOCATION_SUCCESS = 'FETCH_LOCATION_SUCCESS'

export const ASSOCIATE_LOCATION_TO_PLAN_SUCCESS = 'ASSOCIATE_LOCATION_TO_PLAN_SUCCESS';
export const ASSOCIATE_LOCATION_TO_PLAN_FAILURE = 'ASSOCIATE_LOCATION_TO_PLAN_FAILURE';

export const DELETE_ASSOCIATION_FAILURE = 'DELETE_ASSOCIATION_FAILURE';
export const DELETE_ASSOCIATION_SUCCESS = 'DELETE_ASSOCIATION_SUCCESS';

export const CREATE_PLAN_SUCCESS = 'CREATE_PLAN_SUCCESS';
export const CREATE_PLAN_FAILURE = 'CREATE_PLAN_FAILURE'

export const FETCH_PLAN_SUCCESS = 'FETCH_PLAN_SUCCESS';
export const FETCH_PLAN_FAILURE = 'FETCH_PLAN_FAILURE';